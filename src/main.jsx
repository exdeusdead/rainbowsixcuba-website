...
const baseModulesEs = {
  ...
  estadisticas: ['Estadísticas', 'Centro de datos y Companion Extension para conectar estadísticas, API y perfiles públicos.'],
...
const dictionary = {
  ...
  en: {
    ...
    modules: {
      ...
      estadisticas: ['Statistics', 'Data center and Companion Extension for statistics, API and public profiles.'],
  ...
  fr: {
    ...
    modules: {
      comunidad: ['Communauté', 'Connexion et croissance pour les joueurs cubains.'], eventos: ['Événements', 'Scrims, tournois et annonces communautaires.'], competitivo: ['Compétitif', 'Équipes, recrutement, ligues internes et préparation.'], estadisticas: ['Statistiques', 'Companion Extension, API et profils publics en développement.'], coaches: ['Coachs vérifiés', 'Espace futur pour coachs autorisés.'], partners: ['Partenaires vérifiés', 'Organisations, équipes et alliés fiables.'], colaboradores: ['Collaborateurs', 'Aucune communauté ne grandit seule.'], incoming: ['À venir', 'Nouveaux outils et projets en développement.'], privacidad: ['Confidentialité', 'Données protégées et transparence.']
...
const modules = [
  ...
  { id: 'estadisticas', image: '/assets/backgrounds/v08-statistics.webp', icon: BarChart3, accent: 'blue', url: '/statistics.html' },
...
function Header({ t, language, setLanguage }) {
  ...
  const navTargets = ['#inicio','#comunidad','#eventos','#competitivo','/statistics.html','#coaches','#partners','#colaboradores'];
  return (
...
function ModuleCard({ item, t }) {
  ...
  return (
    <a href={item.url || `#${item.id}`} id={item.id} className={`module-card ${item.accent}`}>
